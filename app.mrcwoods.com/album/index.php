    ...
    switch ($action) {
        case 'create_album':
            ...
            $albumDirs = array_filter($allDirs, function($dir) {
                ...
                return !in_array($name, ['回收站']);
            });
            $currentAlbumCount = count($albumDirs);
$maxAlbums = getMaxAlbums();

if ($currentAlbumCount >= $maxAlbums) {
                $errorMsg = "最多可创建{$maxAlbums}个相册，当前已创建{$currentAlbumCount}个";
                break;
            }
