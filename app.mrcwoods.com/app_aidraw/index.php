...
<img src="<?= $record['url'] ?>" class="chat-img" onclick="copyImageUrl('<?= $record['url'] ?>')" data-url="<?= $record['url'] ?>" data-size="<?= $record['size'] ?>" data-prompt="<?= htmlspecialchars($record['prompt']) ?>" title="点击复制链接">
...
async function copyImageUrl(url) {
    try {
        ...
        if (navigator.clipboard && window.isSecureContext) {
            ...
            showModal('success', '复制成功', '图片链接已复制到剪贴板！');
        } else {
            ...
            if (successful) {
                ...
                showModal('error', '复制失败', '请手动复制链接：' + url);
            }
        }
    } catch (err) {
...
const aiMsg = `
    <div class="chat-message ai-message">
        <div class="bubble ai-bubble">
            <div class="bubble-time">刚刚</div>
            <div class="img-container">
<img src="${data.img_url}" class="chat-img" onclick="copyImageUrl('${data.img_url}')" data-url="${data.img_url}" data-size="${size}" data-prompt="${htmlEscape(prompt)}" title="点击复制链接">
...
`;
        ...
        async function saveImage(btn) {
            ...
            try {
                // 调用upload.php接口
                const response = await fetch('../aidraw/upload.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
