...
function validateDrawParams(array $params): array {
    ...
    // 替换为Kolors支持的尺寸
    $allowedSizes = ['1024x1024', '960x1280', '768x1024', '720x1440', '720x1280'];
...
function callAIDrawApi(array $params, array $env): array {
    // 从env获取硅基流动配置
    $apiKey = $env['SILICONFLOW_API_KEY'] ?? '';
    $model = $env['SILICONFLOW_MODEL'] ?? 'Kwai-Kolors/Kolors';
    if (empty($apiKey)) {
        return [false, "硅基流动API密钥未配置"];
    }
    ...
    $requestParams = [
        ...
        'size' => $params['size'], // 直接使用验证后的尺寸（如1024x1024）
        'steps' => $params['steps'],
        'n' => 1, // 生成1张图片，固定值
        'response_format' => 'url' // 直接返回图片URL，简化处理
    ];
    ...
    // 设置请求头，必须带Authorization和Content-Type
    ...
    if ($httpCode !== 200) {
        return [false, "硅基流动API请求失败，状态码：{$httpCode}，响应：{$response}"];
    }
    ...
    if (isset($result['data'][0]['url'])) {
        ...
        $errorMsg = $result['error']['message'] ?? '硅基流动API生成失败，未知错误';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'draw') {
        ...
        [$apiSuccess, $apiData] = callAIDrawApi($params, $env);
        if (!$apiSuccess) {
            echo json_encode(['success' => false, 'msg' => "图片生成失败"]);
            exit;
        }
...
<body>
    ...
    <div class="navbar">
        <div class="navbar-left">
            ...
            <div class="navbar-title">AI绘画Kwai-Kolors/Kolors</div>
        </div>
...
<select id="size" class="form-control">
    <option value="1024x1024" selected>1024 × 1024（1:1，默认）</option>
    <option value="960x1280">960 × 1280（3:4）</option>
    <option value="768x1024">768 × 1024（3:4）</option>
    <option value="720x1440">720 × 1440（1:2）</option>
    <option value="720x1280">720 × 1280（9:16）</option>
                ...
                <button id="generateBtn">生成图片（消耗1积分）</button>
            </div>
        </div>
    </div>
