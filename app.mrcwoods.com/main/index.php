...
<script type="text/javascript">
    ...
    <style>
        ...
        * {
        ...
        .island-nav-btns::-webkit-scrollbar {
            display: none;
        }
        ...
        .content-box.fullscreen .content-iframe {
            border-radius: 0 !important;
        }
    ...
    <nav class="mobile-nav">
        <div class="dynamic-island" id="dynamic-island">
            <div class="island-logo">
                <img src="<?php echo $siteInfo['logo']; ?>" alt="<?php echo $siteInfo['title']; ?>">
            </div>
            <div class="island-nav-btns" id="island-nav-btns">
                <?php foreach ($navMap as $anchor => $item): ?>
                <button class="island-nav-btn" 
                        data-anchor="<?php echo $anchor; ?>"
                        data-target="<?php echo $env['BASE_URL'] . $item['page']; ?>"
                        title="<?php echo $item['name']; ?>">
                    <img src="../svg/<?php echo $item['name']; ?>.svg" alt="<?php echo $item['name']; ?>">
                </button>
                <?php endforeach; ?>
            </div>
        </div>
        ...
        <main class="content-area">
            <div class="content-box" id="content-box">
                ...
                <div class="window-controls" id="window-controls">
                    <button class="window-control-btn close" id="window-close" title="关闭"></button>
                    <button class="window-control-btn minimize" id="window-minimize" title="最小化"></button>
                    <button class="window-control-btn maximize" id="window-maximize" title="最大化"></button>
                </div>
...
if (DOM.logoutbtn) {
    DOM.logoutbtn.addEventListener('click', () => {
        ...
        if (windowClose) {
            windowClose.addEventListener('click', () => {
                if (DOM.contentiframe) {
                    ...
                    DOM.contentiframe.style.display = 'none';
                    DOM.placeholder.style.display = 'flex';
                    DOM.placeholder.textContent = '请点击左侧导航栏选择要访问的页面';
                }
                // 移除全屏状态
                contentBox.classList.remove('fullscreen');
            });
        }
        ...
        // 最大化按钮 - 切换全屏模式并隐藏侧边栏
        if (windowMaximize && contentBox) {
            windowMaximize.addEventListener('click', () => {
                const isFullscreen = contentBox.classList.toggle('fullscreen');
                // 全屏时隐藏侧边栏
        ...
        // ===== 移动端灵动岛导航交互 =====
        const islandNavBtns = document.getElementById('island-nav-btns');
        
        // 灵动岛导航按钮点击处理
        if (islandNavBtns) {
            islandNavBtns.addEventListener('click', (e) => {
                const target = e.target.closest('.island-nav-btn');
                if (!target) return;
                
                // 更新活跃状态
                document.querySelectorAll('.island-nav-btn').forEach(el => {
                    el.classList.remove('active');
                });
                ...
                // 同步侧边栏导航状态
                const sidebarLink = document.querySelector(`.sidebar [data-anchor="${anchor}"]`);
                if (sidebarLink) {
                    setActiveNav(sidebarLink);
                }
                ...
                if (DOM.contentiframe) {
                    ...
                    // 更新URL
                    window.history.pushState({}, '', `${baseUrl}#${anchor}`);
                }
            });
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // 移动端初始化默认页面
            if (window.innerWidth <= 1024 && DOM.contentiframe) {
                const currentHash = window.location.hash.replace('#', '') || 'home';
                const defaultItem = document.querySelector(`.mobile-nav-item[data-anchor="${currentHash}"]`);
                if (defaultItem) {
                    defaultItem.classList.add('active');
                    // 同步底部导航状态
                    document.querySelectorAll(`.mobile-bottom-btn[data-anchor="${currentHash}"]`).forEach(el => {
                        el.classList.add('active');
                    });
                } else {
                    // 默认高亮首页
                    document.querySelector('.mobile-nav-item[data-anchor="home"]')?.classList.add('active');
                    document.querySelector('.mobile-bottom-btn[data-anchor="home"]')?.classList.add('active');
                }
            }
        }, { once: true });
    </script>
