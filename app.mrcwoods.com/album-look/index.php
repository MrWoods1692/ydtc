...
$userData = file_exists($userJsonPath) ? json_decode(file_get_contents($userJsonPath), true) : ['used_space_kb' => 0];
$userUsedSpace = isset($userData['used_space_kb']) ? (float)$userData['used_space_kb'] : 0;
$userMaxSpace = PHP_INT_MAX; // 无限空间 for all users
$userRemainSpace = PHP_INT_MAX; // 无限空间 for all users
...
function formatSize($sizeKb) {
    $sizeKb = $sizeKb ?? 0; // Handle null values
    $units = ['KB', 'MB', 'GB', 'TB'];
    $size = $sizeKb;
    $unitIndex = 0;
