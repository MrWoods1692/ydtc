...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'restore') {
    ...
    if ($file_index >= 0 && isset($trash_files[$file_index])) {
        ...
        // 更新原相册的p.json文件
        $original_p_json = $original_album_dir . 'p.json';
        if (file_exists($original_p_json)) {
            $original_p_data = json_decode(file_get_contents($original_p_json), true) ?: [];
            $original_p_data['last_upload_time'] = time();
            $original_p_data['image_count'] = count($original_files);
            $original_p_data['used_space_kb'] = ($original_p_data['used_space_kb'] ?? 0) + $file_size_kb;
            file_put_contents($original_p_json, json_encode($original_p_data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
        }
...
<body>
    ...
    <div class="navbar">
        <div class="navbar-left">
            <!-- 返回按钮 -->
            <button class="btn-back" onclick="window.location.href='../album/'">
        ...
        <div class="navbar-right">
            <span>已用空间：<?= format_size($used_space_kb) ?></span>
        </div>
    </div>
